...
class GenericStyledAreaBehavior {
    ...
    GenericStyledAreaBehavior(GenericStyledArea<?, ?, ?> area, GenericStyledAreaBehaviorParameters parameters) {
        this.view = area;

        InputMapTemplate.installFallback(generateInputMapTemplate(parameters), this, b -> b.view);
    ...
    public InputMapTemplate<GenericStyledAreaBehavior, ? super Event> generateInputMapTemplate(
            GenericStyledAreaBehaviorParameters parameters) {
        ...
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> copy;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> cut;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> paste;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> undo;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> redo;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> redoShift;
        InputMapTemplate<GenericStyledAreaBehavior, KeyEvent> selectAll;


        copy = consume(parameters.isIncludeCopy() ?
                        anyOf(keyPressed(COPY), keyPressed(SHORTCUT_C), keyPressed(INSERT, SHORTCUT_DOWN)) :
                        anyOf(keyPressed(COPY), keyPressed(INSERT, SHORTCUT_DOWN)),
                (b, e) -> b.view.copy());

        cut = consume(parameters.isIncludeCut() ?
                        anyOf(keyPressed(CUT), keyPressed(SHORTCUT_X), keyPressed(DELETE, SHIFT_DOWN)) :
                        anyOf(keyPressed(CUT), keyPressed(DELETE, SHIFT_DOWN)),
                (b, e) -> b.view.cut());

        paste = consume(parameters.isIncludePaste() ?
                        anyOf(keyPressed(PASTE), keyPressed(SHORTCUT_V), keyPressed(INSERT, SHIFT_DOWN)) :
                        anyOf(keyPressed(PASTE), keyPressed(INSERT, SHIFT_DOWN)),
                (b, e) -> b.view.paste());

        undo = consume(keyPressed(SHORTCUT_Z), (b, e) -> b.view.undo());
        redo = consume(keyPressed(SHORTCUT_Y), (b, e) -> b.view.redo());
        redoShift = consume(keyPressed(SHORTCUT_SHIFT_Z), (b, e) -> b.view.redo());
        selectAll = consume(keyPressed(SHORTCUT_A), (b, e) -> b.view.selectAll());
        ...
        if (parameters.isIncludeSelectAll()) keyPressedTemplate = keyPressedTemplate.orElse(selectAll);
        if (parameters.isIncludeUndo()) keyPressedTemplate = keyPressedTemplate.orElse(undo);
        if (parameters.isIncludeRedo()) keyPressedTemplate = keyPressedTemplate.orElse(redo);
        if (parameters.isIncludeShiftZRedo()) keyPressedTemplate = keyPressedTemplate.orElse(redoShift);

        keyPressedTemplate = keyPressedTemplate.ifConsumed((b, e) -> b.view.requestFollowCaret())
        ...
        return sequence(mouseTemplate, keyPressedTemplate, keyTypedTemplate, contextMenuEventTemplate);
    }
}
